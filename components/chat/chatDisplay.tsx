...
export function ChatDisplay({
  ...
  useEffect(() => {
    ...
    socket.on(
      "messagesSeen",
      ({ userId, lastSeenMessageId, updatedMessages }: any) => {
        setMessages((prevMessages: any) =>
          prevMessages.map((msg: any) =>
            msg.id <= lastSeenMessageId
              ? updatedMessages.find(
                  (updatedMsg: Message) => updatedMsg.id === msg.id
                ) || msg
              : msg
          )
        );
  ...
  const roomId =
    conversationType === "PRIVATE" ? `room_${doctorId}_${clientId}` : convoId;
  useEffect(() => {
  ...
  const uploadFile = async () => {
    ...
    try {
      const res = await fetch(
        "https://chatapi.kaustubhamedtech.com/api/upload",
        {
          method: "POST",
          body: formData,
        }
      ...
      <div className="flex relative  h-full flex-col">
        {isVideoCallActive ? (
          ...
          <>
                    ...
                    return (
                      <div key={item.id}>
                        ...
                        {isLastMessage &&
                          (() => {
                            const targetUserId =
                              role === "DOCTOR" ? clientId : doctorId;
                            const seenTime = item.seenBy?.find(
                              (seen: any) => seen.userId === targetUserId
                            )?.seenAt;
            ...
            {((conversationType === "PRIVATE" &&
              confirmedAppointments.length > 0) ||
              (role == "DOCTOR" && conversationType === "COMMUNITY")) && (
