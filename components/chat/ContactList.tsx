...
const ContactList = ({
  ...
  const GetAllDoctorsWithDetails = async () => {
    ...
    const doctors = Array.isArray(result) && result.length > 0 ? result[0] : [];
    const doctorIds = doctors.map((doc: any) => ({
      id: doc.id,
      name: doc.name,
      role: doc.role,
    }));
  ...
  const [filteredSuggestions, setFilteredSuggestions] =
    useState(doctorsAndPatients);

  useEffect(() => {
    setFilteredSuggestions(
      combinedData.filter((person:any) =>
