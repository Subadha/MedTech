...
const ContactList = ({
  ...
  const handleRemoveMember = (index: number) => {
    ...
    setcomunitymembersNames((prev: any) =>
  ...
  const handleCreateCommunity = () => {
    ...
    socket.emit("createCommunity", {
      newCommunityName,
      communityMembers: updatedMembers,
    });
  ...
  if (role != "DOCTOR") {
    return (
      <div className="h-full w-full">
        <div className="flex justify-center p-4">
          ...
          >
            {isCommunityView
              ? "Switch to User Chats"
              : "Switch to Community Chats"}
          </Button>
        </div>
              ...
              {items?.map((item:any, index) => (
                <button
                  ...
                  className={cn(
                    ...
                    mail.selected !== item.doctor_id && "hover:bg-muted",
                    mail.selected === item.doctor_id && "bg-primary"
                  )}
                  onClick={() =>
                    setMail({
                      ...mail,
                    ...
                    <div className="flex gap-2">
                      <Avatar className="h-12 w-12">
                        <AvatarImage
                          ...
                          src={item?.doctor?.image||`https://avatar.iran.liara.run/username?username=${
                            item.doctorName || "Default Name"
                          }`}
                          alt="@shadcn"
                        />
                      ...
                      <div
                        className={`flex w-[calc(100%-70px)] ${
                          mail.selected ? "" : "border-b"
                        } mt-1 gap-2`}
                      >
                        <div>
                          <h3
                      ...
                      <Avatar className="h-12 w-12">
                        <AvatarImage
                          ...
                          src={`https://avatar.iran.liara.run/username?username=${
                            item.CommunityName || "Default Name"
                          }`}
                          alt="@shadcn"
                        />
                        <AvatarFallback>CN</AvatarFallback>
    ...
    return (
      <div className="h-full w-full">
        ...
        <Dialog
          open={isCreatingCommunity}
          onOpenChange={setIsCreatingCommunity}
        >
          <DialogContent>
            <DialogHeader>
            ...
            <div className="origin-top-right flex flex-col mt-4 p-3 pt-0">
              {items?.map((item:any, index) => (
                <button
                  ...
                  className={cn(
                    "flex flex-col items-start gap-2 rounded-lg p-2 text-left text-sm transition-all",
                    mail.selected !== item.userId && "hover:bg-muted",
                    mail.selected === item.userId && "bg-primary"
                  )}
                  onClick={() => {
                    setMail({
                      ...mail,
                  ...
                  <div className="flex w-full flex-col gap-1">
                    <div className="flex gap-2">
                      <Avatar className="h-12 w-12">
                        <AvatarImage
                          className="rounded-full"
                          src={item?.patient?.image||`https://avatar.iran.liara.run/username?username=${
